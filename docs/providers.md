...
| Capability | `OllamaProvider` | `OpenAICompatibleProvider` | `FakeProvider` (full) | `FakeProvider` (minimal) | What declaring it buys a caller |
|---|---|---|---|---|---|
| `streaming` | yes | yes | yes | no | `stream()` yields deltas; otherwise it refuses. |
| `tool_calling` | yes | yes | yes | no | `tools=` is accepted and calls can be requested. |
| `structured_output` | yes | yes | yes | no | `ResponseFormat.JSON_SCHEMA` is enforced. |
| `json_mode` | yes | yes | yes | no | `ResponseFormat.JSON` is honoured without a schema. |
| `token_counts` | yes | yes | yes | no | Token counts are real; otherwise every count is `UNSUPPORTED`. |
| `token_level_chunks` | yes | no | yes | no | **Gates any per-token latency claim.** When `False`, the gap between two deltas is inter-chunk latency and must not be relabelled. |
| `thinking_control` | yes | no | yes | no | Reasoning output can be requested or suppressed. |
| `logprobs` | no | no | no | no | Per-token log probabilities are reported. |
| `force_unload` | yes | no | yes | no | `load()` and `unload()` act; otherwise both refuse. |
| `residency_query` | yes | no | yes | no | `list_resident()` answers; otherwise it refuses. |
| `kv_metrics` | no | no | no | no | KV-cache metrics are reported. |
| `context_configurable` | yes | no | yes | no | **Load-bearing.** Whether you may set a served context, or must record the one you got as *assumed*. |
| `embedding` | no | no | no | no | Embeddings can be produced. Out of scope until spec §21. |
