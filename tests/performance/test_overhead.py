"""Spec §15's **timing** budgets, measured against a transport that costs nothing.

Marked ``performance`` and excluded from the default run (``addopts``), because a wall-clock
assertion on shared CI hardware is a flake generator, not a gate. Run deliberately, or from the
nightly job:

    pytest -m performance

Spec §15's fourth budget — memory per active stream — is deliberately **not** here. It is measured
as a live allocation set rather than against a clock, which makes it deterministic, which makes
excluding it from the default run a cost with no matching benefit: it lives in
``tests/unit/test_stream_memory.py`` and runs on every push. The marker is for measurements a
loaded machine can distort, not for the word "performance".
