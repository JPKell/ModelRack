...
### Packaging
- `requirements/{ci.lock,release.in,release.lock,README.md}`. The repository had an empty
  `requirements/` holding a `.gitkeep`, so `release.yml`'s TestPyPI dry run failed at its first
  step with `No such file or directory: 'requirements/release.lock'`. Both locks are hash-verified
  and resolved on Python 3.13, matching the convention BaseAiCore and SweatMeter published under
  (Packaging and Release Standards §4, Security Standards §11). The placeholder `.gitkeep` they
  replace claimed the locks were "generated by CI", which is backwards — a lock CI generated for
  itself is a lock that verifies nothing. They are generated locally and committed.
- Every blocking CI job now installs `--require-hashes -r requirements/ci.lock` and then
  `pip install . --no-deps`, rather than re-resolving `.[dev]` on each run. `format` and `lint`
  included: a ruff release changes formatting, and an unpinned format job turns that into a red
  build with no commit behind it.
- The `security` job audits the locked sets — `pip-audit --require-hashes -r requirements/ci.lock`
  and the same for `release.lock` — instead of running a bare `pip-audit` against an environment
  containing only `pip-audit` itself, which is what it had been doing and which could never have
  found anything.
- CI's `build` job and the `release` job both build from `release.lock` with `--no-isolation`,
  the same chain the TestPyPI dry run uses. A release that resolved `build` and `hatchling` fresh
  from PyPI would not be the artifact the dry run proved.

### Fixed (release pipeline)
- **The `release` job would have published to real PyPI during a TestPyPI dry run.** Adding
  `workflow_dispatch` to the workflow's triggers arms every job in it, so clicking "Run workflow"
  for the dry run would have fired the real publish alongside it. Now guarded by
  `if: github.event_name == 'push'`, as SweatMeter's equivalent already was.
- **Coverage measured nothing under CI's install mode.** `[tool.coverage.run] source` named the
  path `src/modelrack`, but CI installs the built distribution rather than an editable checkout,
  so the imported package lives in `site-packages` and the configured path matches nothing:
  932 tests pass, 0 % is reported, and the `--cov-fail-under=95` gate fails with a number that
  looks like a catastrophe instead of a configuration error. Now `source = ["modelrack"]` with a
  `[tool.coverage.paths]` mapping folding the two locations together — the same fix BaseAiCore and
  SweatMeter already carry, and the reason only the two published repositories had it is that only
  they had ever run CI against a non-editable install. Verified at 100 % in both modes.
...
### Notes
- Coverage is 100 % of statements and branches, against a floor of 95 %, measured both from an
